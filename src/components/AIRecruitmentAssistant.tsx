...
const AIRecruitmentAssistant: React.FC<AIRecruitmentAssistantProps> = ({
  ...
  const generateJobDescription = async (roleTitle: string) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-jd', {
        body: {
          roleTitle,
          companyName
        }
      ...
      addMessage('assistant', `I couldn't reach Gemini right now, so I created a solid starter JD for **${roleTitle}**. You can apply it or ask me to refine it.`, {
        jobData: {
          description: fallbackDesc,
          skills: fallbackSkills
        }
      ...
      setConversationState('jd_ready');
      toast({
        title: 'Temporary fallback used',
        description: 'Gemini API was unreachable. Using a basic JD.',
        variant: 'destructive'
      });
    } finally {
  ...
  return <Card className="shadow-soft h-[600px] flex flex-col">
      ...
      <CardContent className="flex-1 flex flex-col p-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          {messages.map(message => <div key={message.id} className={`flex gap-2 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex gap-2 max-w-[80%] ${message.type === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${message.type === 'user' ? 'bg-primary text-primary-foreground' : 'bg-accent text-accent-foreground'}`}>
                  {message.type === 'user' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                </div>
                <div className={`rounded-lg p-2 ${message.type === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                  <div className="whitespace-pre-wrap text-sm bg-stone-200">{message.content}</div>
                  {message.data?.jobData && <div className="mt-3 pt-3 border-t border-border/20">
                      <Button size="sm" onClick={applyJobDescription} className="bg-success text-success-foreground hover:bg-success/90">
          ...
          {isProcessing && <div className="flex gap-2 justify-start">
        ...
        <div className="border-t p-4">
          <div className="flex gap-2">
            <Textarea value={currentInput} onChange={e => setCurrentInput(e.target.value)} onKeyPress={handleKeyPress} placeholder={conversationState === 'initial' ? "Tell me the role you're hiring for (e.g., Senior Software Engineer)..." : "Ask me to modify the JD, apply it, or generate a new one..."} className="flex-1 min-h-[40px] max-h-[100px] resize-none" disabled={isProcessing} />
            <Button onClick={handleSendMessage} disabled={!currentInput.trim() || isProcessing} size="sm" className="px-4 mx-0 my-[12px]">
              Send
            </Button>
          </div>
        </div>
